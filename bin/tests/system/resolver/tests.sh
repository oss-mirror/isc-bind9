...
$DIG $DIGOPTS @10.53.0.5 should.not.be.logged > dig.ns5.out.${n} || ret=1
...
$DIG $DIGOPTS @10.53.0.5 should.be.logged > dig.ns5.out.${n} || ret=1
grep "response: should.be.logged" ns5/named.run > /dev/null || ret=1
if [ $ret != 0 ]; then echo_i "failed"; fi
status=`expr $status + $ret`
...
$DIG $DIGOPTS @10.53.0.1 should.be.logged > dig.ns1.out.${n} || ret=1
grep "response: should.be.logged" ns1/named.run > /dev/null || ret=1
if [ $ret != 0 ]; then echo_i "failed"; fi
status=`expr $status + $ret`
...
$DIG $DIGOPTS @10.53.0.6 should.be.logged > dig.ns6.out.${n} || ret=1
grep "response: should.be.logged" ns6/named.run > /dev/null || ret=1
if [ $ret != 0 ]; then echo_i "failed"; fi
status=`expr $status + $ret`
