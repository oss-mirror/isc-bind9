...
static isc_result_t
bind9_check_tls_defintion(const cfg_obj_t *tlsobj, isc_log_t *logctx,
			  isc_symtab_t *symtab, isc_mem_t *mctx) {
	isc_result_t result, tresult;
	...
	if (strcmp(name, "ephemeral") == 0 || strcmp(name, "none") == 0) {
		cfg_obj_log(tlsobj, logctx, ISC_LOG_ERROR,
...
bind9_check_tls_definitions(const cfg_obj_t *config, isc_log_t *logctx,
	...
	for (elt = cfg_list_first(obj); elt != NULL; elt = cfg_list_next(elt)) {
		obj = cfg_listelt_value(elt);
		tresult = bind9_check_tls_defintion(obj, logctx, symtab, mctx);
		if (result == ISC_R_SUCCESS) {
			result = tresult;
		}
	}
