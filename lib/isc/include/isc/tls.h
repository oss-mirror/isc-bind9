...
typedef enum isc_tls_protocol_version {
	...
	ISC_TLS_PROTO_VER_1_2 = 1,
	ISC_TLS_PROTO_VER_1_3 = 2,
...
/*%<
 * Check if cipher list string for TLSv1.2 is valid.
 *
 * Requires:
...
/*%<
 * Set cipher list string for TLSv1.2 on the given TLS context 'ctx'.
 *
 * Requires:
 * \li	'ctx' != NULL;
 * \li	'cipherlist' a valid pointer to a non empty string.
