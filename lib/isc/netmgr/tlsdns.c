...
isc_nm_listentlsdns(isc_nm_t *mgr, isc_nmiface_t *iface,
...
#if !HAVE_SO_REUSEPORT_LB && !defined(WIN32)
	sock->fd = isc__nm_tlsdns_lb_socket(iface->addr.type.sa.sa_family);
#else
	sock->fd = -1;
#endif
	...
	for (size_t i = 0; i < sock->nchildren; i++) {
		...
		start_tlsdns_child(mgr, iface, sock, i);
	}

	if (isc__nm_in_netthread()) {
		start_tlsdns_child(mgr, iface, sock, isc_nm_tid());
	}
...
isc__nm_async_tlsdnslisten(isc__networker_t *worker, isc__netievent_t *ev0) {
	...
	uv_barrier_wait(&sock->mgr->startlistening);
}
...
stop_tlsdns_child(isc_nmsocket_t *sock) {
	...
	(void)uv_barrier_wait(&sock->mgr->stoplistening);
}
