...
isc__netmgr_create(isc_mem_t *mctx, uint32_t workers, isc_nm_t **netmgrp) {
	...
	*mgr = (isc_nm_t){ .nworkers = workers };
	...
	uv_barrier_init(&mgr->startlistening, workers);
	uv_barrier_init(&mgr->stoplistening, workers);
	uv_barrier_init(&mgr->pausing, workers);
	uv_barrier_init(&mgr->resuming, workers);
...
nm_destroy(isc_nm_t **mgr0) {
	...
	*mgr0 = NULL;
	...
	uv_barrier_destroy(&mgr->resuming);
	uv_barrier_destroy(&mgr->pausing);
	uv_barrier_destroy(&mgr->stoplistening);
	uv_barrier_destroy(&mgr->startlistening);
...
nm_thread(isc_threadarg_t worker0) {
	...
	while (true) {
		...
		if (worker->paused) {
			...
			if (uv_barrier_wait(&mgr->pausing) > 0) {
				LOCK(&mgr->lock);
			...
			if (uv_barrier_wait(&mgr->resuming) > 0) {
				LOCK(&mgr->lock);
				SIGNAL(&mgr->wkstatecond);
				UNLOCK(&mgr->lock);
			}
