...
raw_key_to_ossl(unsigned int key_alg, int private, const unsigned char *key,
	...
	if (private) {
		...
		memcpy(buf + 1, key, len);

		if (OSSL_PARAM_BLD_push_octet_string(
			    bld, OSSL_PKEY_PARAM_PUB_KEY, buf, 1 + len) != 1)
		{
			OSSL_PARAM_BLD_free(bld);
			DST_RET(ret);
		}
...
opensslecdsa_generate(dst_key_t *key, int unused, void (*callback)(int)) {
	...
	status = EVP_PKEY_paramgen(ctx, &params);
	if (status != 1) {
...
#else
	ret = (EVP_PKEY_get_bn_param(key->keydata.pkey,
...
ecdsa_check(EVP_PKEY **pkey, EVP_PKEY *pubpkey) {
	...
	if (EVP_PKEY_get_bn_param(pubpkey, OSSL_PKEY_PARAM_EC_PUB_X, &x) != 1 ||
	    x == NULL ||
	...
	if (status != 1 || pkey_len == 0) {
		/*
		 * We can't add the public key to the existing PKEY, so we have
		 * to create a new PKEY
		 */
...
opensslecdsa_fromlabel(dst_key_t *key, const char *engine, const char *label,
	...
	char buf[128]; /* Plenty for all of the supported curves' names  */
#endif /* OPENSSL_VERSION_NUMBER < 0x30000000L */
	EVP_PKEY *pkey = NULL;
...
#else
	if (ecdsa_check(&pkey, pubkey) != ISC_R_SUCCESS) {
		DST_RET(DST_R_INVALIDPRIVATEKEY);
	}
#endif /* OPENSSL_VERSION_NUMBER < 0x30000000L */
...
opensslecdsa_parse(dst_key_t *key, isc_lex_t *lexer, dst_key_t *pub) {
	...
	if (label != NULL) {
		...
		result = raw_key_to_ossl(key->key_alg, 1,
					 priv.elements[privkey_index].data,
					 &len, &key->keydata.pkey);
#endif /* OPENSSL_VERSION_NUMBER < 0x30000000L */
	...
	if (finalize_key) {
		result = finalize_eckey(key, NULL, engine, label);
	}
#endif /* OPENSSL_VERSION_NUMBER < 0x30000000L */
